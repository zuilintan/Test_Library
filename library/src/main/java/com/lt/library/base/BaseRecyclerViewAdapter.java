...
public abstract class BaseRecyclerViewAdapter<DS, VH extends ViewHolder> extends Adapter<VH>
        ...
        switch (viewHolder.getItemViewType()) {
            case VIEW_TYPE_ITEM:
                if (mHeadViewId == -1) {
                    ...
                    init(viewHolder, mDataSourceList.get(position),
                         position);
                }//无HeadView时的处理
                else {
                    viewHolder.itemView.setTag(position - 1);
                    init(viewHolder, mDataSourceList.get(position - 1),
                         (position - 1));
                }//有HeadView时的处理
                break;
            case VIEW_TYPE_HEAD:
    ...
    @Override
    public void onBindViewHolder(@NonNull VH viewHolder,
                                 int position,
            ...
            switch (viewHolder.getItemViewType()) {
                case VIEW_TYPE_ITEM:
                    if (mHeadViewId == -1) {
                        ...
                        viewHolder.itemView.setTag(position - 1);
                        updateChildView(viewHolder,
                                        mDataSourceList.get(position - 1),
                                        position - 1,
                                        payloads);
                    }//有HeadView时的处理
                    break;
    ...
    public void addDS(DS dataSource, int position) {
    ...
    public void delDS(int position) {
        int size = mDataSourceList.size();
    ...
    public void updateDS(DS dataSource, int position, String payload) {
    ...
    }//更新指定位置的数据源，并可以单独更新其中的某个View

    public void clearDSList() {
        int size = mDataSourceList.size();
    ...
    public void refreshDSList(List<DS> dataSourceList) {
        int size = mDataSourceList.size();
        if (mHeadViewId == -1) {
            mDataSourceList.clear();
            notifyItemRangeRemoved(0, size);
    ...
    public void loadMoreDSList(List<DS> dataSourceList) {
        int size = mDataSourceList.size();
        mDataSourceList.addAll(dataSourceList);
        if (mHeadViewId == -1) {
    ...
    protected abstract void init(VH vh, DS ds, int position);

    protected abstract void updateChildView(VH vh,
                                            DS ds,
                                            int position,
